...
[task_local]
#京东宠汪汪
15 0,6-23/2 * * * jd_joy_new.js, tag=京东宠汪汪, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdcww.png, enabled=true

============Loon===========
[Script]
cron "15 0,6-23/2 * * *" script-path=jd_joy_new.js,tag=京东宠汪汪

============Surge==========
[Script]
京东宠汪汪 = type=cron,cronexp="15 0,6-23/2 * * *",wake-system=1,timeout=3600,script-path=jd_joy_new.js

===============小火箭==========
京东宠汪汪 = type=cron,script-path=jd_joy_new.js, cronexpr="15 0,6-23/2 * * *", timeout=3600, enable=true
*/
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      for (let tp of tasks.datas) {
        ...
        if (tp.taskName === '每日签到' && tp.receiveStatus === 'chance_left') await sign();
    ...
    $.post(
      {
        ...
        headers: {
          ...
          referer: 'https://h5.m.jd.com/',
          'Content-Type':
            fnId === 'followGood' ? 'application/x-www-form-urlencoded' : 'application/json; charset=UTF-8',
          'User-Agent': $.isNode()
            ? process.env.JD_USER_AGENT
              ? process.env.JD_USER_AGENT
              : require('./USER_AGENTS').USER_AGENT
