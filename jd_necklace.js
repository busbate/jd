...
Last Modified time: 2021-06-23 17:27:14
...
class ZooFakerNecklace {
  ...
  async run(data) {
    ...
    const pin = decodeURIComponent(
      this.cookie.match(/pt_pin=([^; ]+)(?=;?)/) && this.cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]
    );
...
function showMsg() {
  return new Promise(async (resolve) => {
    ...
    if (
      ...
      (process.env.DDQ_NOTIFY_CONTROL ? process.env.DDQ_NOTIFY_CONTROL === 'false' : !!1)
    ) {
            ...
            if (data.rtn_code === 0) {
              if (data.data.biz_code === 0) {
                ...
                message += `红包有效期：${new Date(
                  result.endTime + new Date().getTimezoneOffset() * 60 * 1000 + 8 * 60 * 60 * 1000
  ...
  return new Promise((resolve) => {
    if (functionId === 'getCcTaskList') {
      url = `https://api.m.jd.com/client.action?functionId=${functionId}&body=${escape(
        JSON.stringify(body)
      )}&uuid=8888888&client=apple&clientVersion=9.4.1&st=1614320848090&sign=d3259c0c19f6c792883485ae65f8991c&sv=111`;
    }
    if (type === '3' && functionId === 'reportCcTask')
      url = `https://api.m.jd.com/client.action?functionId=${functionId}&body=${escape(
        JSON.stringify(body)
      )}&uuid=8888888&client=apple&clientVersion=9.4.1&st=1622194121039&sign=d565c4594b8e05645f1fe9a495ac7a7d&sv=122`;
    if (type === '4' && functionId === 'reportCcTask')
      url = `https://api.m.jd.com/client.action?functionId=${functionId}&body=${escape(
        JSON.stringify(body)
      )}&uuid=8888888&client=apple&clientVersion=9.4.1&st=1622193986049&sign=f5abd9fd7b9b8abaa25b34088f9e8a54&sv=102`;
...
function taskPostUrl(function_id, body = {}) {
  ...
  return {
    url: `${JD_API_HOST}?functionId=${function_id}&appid=coupon-necklace&loginType=2&client=coupon-necklace&t=${time}&body=${escape(
      JSON.stringify(body)
    )}&uuid=88732f840b77821b345bf07fd71f609e6ff12f43`,
